...
# Registering the new operator and panel
def register():
...
if __name__ == "__main__":
    ...
    bl_idname = "scene.duplicate_all_as_collision"
    bl_label = "Duplicate All as Collision"
    bl_description = "Duplicate all objects in the scene as collision meshes with '.ColMesh' suffix and organize them above the original objects"
    bl_options = {'REGISTER', 'UNDO'}
    ...
    def execute(self, context):
        ...
        # Get a list of all original objects in the scene
        original_objects = sorted(context.scene.objects, key=lambda obj: obj.name)

        # A list to store duplicated objects for reordering
        duplicated_objects = []
        ...
        for obj in original_objects:
            ...
            # Link the duplicate to the same collection as the original object
            for collection in obj.users_collection:
                collection.objects.link(duplicate)

            # Add the duplicate to the duplicated_objects list for reordering later
        ...
        # Organize the objects by moving the duplicates above the original objects in the same collection
        for obj in original_objects:
            for collection in obj.users_collection:
                duplicate_name = f"{obj.name}.ColMesh"
                duplicate_obj = collection.objects.get(duplicate_name)
                if duplicate_obj:
                    # Unlink and relink objects to move the duplicate above the original
                    collection.objects.unlink(duplicate_obj)
                    collection.objects.link(duplicate_obj)

        # Deselect all objects after operation
        bpy.ops.object.select_all(action='DESELECT')

        self.report({'INFO'}, "Duplicated and organized all objects as collision meshes")
        return {'FINISHED'}
    bl_idname = "scene.duplicate_all_as_collision"
    bl_label = "Duplicate All as Collision"
    bl_description = "Duplicate all objects in the scene as collision meshes with '.ColMesh' suffix, place them in their own collections, and organize them above the original"
    bl_options = {'REGISTER', 'UNDO'}
    ...
    def execute(self, context):
        ...
        for obj in original_objects:
            ...
            # Create a new collection for the ColMesh object, named after the original object
            colmesh_collection_name = f"{obj.name}.ColMesh"
            colmesh_collection = bpy.data.collections.new(colmesh_collection_name)
            context.scene.collection.children.link(colmesh_collection)

            # Link the duplicate to its own new collection
            colmesh_collection.objects.link(duplicate)
        ...
        # Sort both original and duplicated objects by name to place duplicated objects above originals
        all_objects_sorted = sorted(original_objects + duplicated_objects, key=lambda obj: obj.name.lower(), reverse=True)

        # Unlink and relink objects in order to reorder them in the outliner
...
class SCENE_PT_collision_tools(bpy.types.Panel):
    ...
    bl_idname = "scene.duplicate_all_as_collision"
    bl_label = "Duplicate All as Collision"
    bl_description = "Duplicate all objects in the scene as collision meshes with '.ColMesh' suffix and organize them in their own collection"
    bl_options = {'REGISTER', 'UNDO'}

    def execute(self, context):
        # Create a new collection for the collision meshes
        colmesh_collection = bpy.data.collections.new("ColMesh_Objects")
        context.scene.collection.children.link(colmesh_collection)
        ...
        for obj in original_objects:
            ...
            # Link duplicate to the new ColMesh_Objects collection
            colmesh_collection.objects.link(duplicate)
        ...
        # Relink objects for proper order in the outliner
        for collection in context.scene.collection.children:
            for obj in all_objects_sorted:
                if obj in collection.objects:
                    collection.objects.unlink(obj)
    ...
    bl_idname = "scene.duplicate_all_as_collision"
    bl_label = "Duplicate All as Collision"
    bl_description = "Duplicate all objects in the scene as collision meshes with '.ColMesh' suffix and organize them"
    bl_options = {'REGISTER', 'UNDO'}
    ...
    def execute(self, context):
        ...
        for obj in original_objects:
            ...
            # Link duplicate to the same collections as the original object
            for collection in obj.users_collection:
                collection.objects.link(duplicate)
        ...
        # Organize the objects based on their names
        # Sort both original and duplicated objects by name to place duplicated objects above originals
        all_objects_sorted = sorted(original_objects + duplicated_objects, key=lambda obj: obj.name.lower(), reverse=True)
