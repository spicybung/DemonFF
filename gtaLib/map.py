        ...
        def export_to_ide(file_path, objects):
            ...
            with open(file_path, 'w') as f:
                ...
                for obj in objects:
                    ...
                    line = f"{object_id}, {name_mapping[base_name]}, {txd_name}, 1, {obj.get('DrawDistance', 300.0)}, 0\n"
                    f.write(line)
    ...
    def execute(self, context):
        def export_to_pawn(file_path, objects):
            ...
            with open(file_path, 'w') as f, open(artconfig_path, 'w') as artconfig:
                ...
                min_id = -30000  # Minimum ID

                artconfig.write("virtualworld, baseid, newid, dffname[], txdname[]\n")

                name_mapping = {}
                for obj in objects:
                    if current_id < min_id:
                        self.report({'ERROR'}, "Minimum ID limit reached. Export cancelled.")
                        break
                    ...
                    line = f"CreateDynamicObject({object_id}, {position.x:.6f}, {position.y:.6f}, {position.z:.6f}, " \
                           f"{rotation[0]:.6f}, {rotation[1]:.6f}, {rotation[2]:.6f}, {interior}, 0, -1, {stream_distance:.2f}, {draw_distance:.2f}, -1, 0);\n"
                    f.write(line)
                    print(f"Exporting {obj.name} with ID {object_id}")

                    # Write to artconfig.txt
                    artconfig_line = f"-1, {object_id}, {object_id}, {dff_name}, {txd_name}\n"
...
class MapExportPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row.operator("object.export_to_pawn", text="Export to Pawn Script")
...
class DemonFFPawnPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        layout = self.layout
        row = layout.row()
        row.operator("object.export_to_pawn", text="Export Pawn Script")
