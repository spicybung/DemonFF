...
class OBJECT_PT_dffObjects(bpy.types.Panel):
    ...
    def draw_col_menu(self, context):
        ...
        self.draw_pipeline_menu(context, box)

    #######################################################
    def draw_pipeline_menu(self, context, box):
        settings = context.object.dff

        box.label(text="Pipeline Settings")
        box.prop(settings, "pipeline", text="Pipeline")
        if settings.pipeline == 'CUSTOM':
            col = box.column()
            col.alert = not self.validate_pipeline(settings.custom_pipeline)
...
def register():
    ...
    bpy.types.TOPBAR_MT_file_import.append(import_dff_func)
    bpy.types.TOPBAR_MT_file_export.append(export_dff_func)
...
def unregister():
    ...
    bpy.types.TOPBAR_MT_file_import.remove(import_dff_func)
    bpy.types.TOPBAR_MT_file_export.remove(export_dff_func)
