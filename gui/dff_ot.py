...
class EXPORT_OT_dff_custom(bpy.types.Operator, ExportHelper):
    ...
    filter_glob: bpy.props.StringProperty(default="*.dff;*.col;*.zip",
                                          options={'HIDDEN'})
...
class IMPORT_OT_dff_custom(bpy.types.Operator, ImportHelper):
    ...
    skip_empty_col_models: bpy.props.BoolProperty(
        name="Skip Empty COL Models",
        description="When importing COL files or COL archives, skip truly empty placeholder entries",
        default=True
    )

    organize_col_by_dff: bpy.props.BoolProperty(
        name="Organize COL By DFF",
        description="When importing COL files or COL archives, put matching collision collections under their DFF collection",
        default=True
    )
    ...
    def draw(self, context):
        ...
        layout.prop(self, "skip_empty_col_models")
        layout.prop(self, "organize_col_by_dff")
    #######################################################    
    def execute(self, context):
        start = time.time()

        files_to_import = [os.path.join(self.directory, file.name) for file in self.files] if self.files else [self.filepath]
        dff_files = []
        col_sources = []

        for file in files_to_import:
            if zipfile.is_zipfile(file) or file.lower().endswith(".col"):
                col_sources.append(file)
            else:
                dff_files.append(file)

        # Import DFFs first so COL libraries can be organized under the matching DFF collections.
        ...
        for file in col_sources:
            if zipfile.is_zipfile(file):
                imported_collections = col_importer.import_col_zip_file(
                    file,
                    os.path.splitext(os.path.basename(file))[0],
                    True,
                    self.skip_empty_col_models,
                    self.organize_col_by_dff
                )
            else:
                imported_collections = col_importer.import_col_file(
                    file,
                    os.path.splitext(os.path.basename(file))[0],
                    True,
                    self.skip_empty_col_models,
            ...
            print(
                "DemonFF DFF/Col import: imported %d COL collection(s) from %s." % (
                    len(imported_collections),
                    file
                )
            )
