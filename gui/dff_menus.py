...
from .col_ot import EXPORT_OT_col, IMPORT_OT_col
from .ifp_ot import IMPORT_OT_ifp
from .mdl_ot import IMPORT_OT_mdl_custom
from ..ops import col_importer
...
#######################################################
class OBJECT_OT_import_col_info(bpy.types.Operator, ImportHelper):
    bl_idname = "object.import_col_info"
    bl_label = "Import COL Info"
    bl_description = "Overwrite material/surface/light flags on existing collision objects from COL files or COL archives"
    filename_ext = ""
        ...
        for path in paths:
            ...
            if os.path.isdir(path):
                for name in os.listdir(path):
                    ...
                    if lower_name.endswith(".col") or lower_name.endswith(".zip"):
                        expanded_paths.append(os.path.join(path, name))
                continue
    ...
    def execute(self, context):
        ...
        if not paths:
            self.report({'ERROR'}, "No COL files selected")
            return {'CANCELLED'}
        ...
        for path in paths:
            if not os.path.exists(path):
                ...
                print("DemonFF Import COL Info: missing source %s" % path)
                continue

            imported = col_importer.apply_col_info_from_file(
                path,
                self.only_selected
            )
        ...
        if missing_paths:
            self.report({'WARNING'}, "Skipped %d missing COL source(s)" % len(missing_paths))

        if imported_total <= 0:
            self.report({'WARNING'}, "No matching collision objects found")
            return {'CANCELLED'}

        self.report(
            {'INFO'},
            "Applied COL info to %d object(s) from %d source(s)" % (
                imported_total,
                sources_used
            )
        )
...
class OBJECT_PT_dff_misc_panel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        layout.operator("object.import_col_info", text="Import COL Info")
...
class DFF_MT_ImportChoice(bpy.types.Menu):
    ...
    def draw(self, context):
        ...
        layout.operator(IMPORT_OT_col.bl_idname,
                        text="DemonFF COL (.col)")
